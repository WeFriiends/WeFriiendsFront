...
interface ChatState {
  ...
  openConversation: (otherUserId: string, conversationRef: string) => void
  ...
  devtools(
    (set, get) => {
      ...
      return {
        ...
        setSelectedChatId: (chatId) => {
          ...
          if (chatId) {
            ...
            if (conversation && conversation.conversationRef) {
              activateConversation(chatId, conversation.conversationRef)
            }
            // Not found yet (e.g. brand new chat) — use openConversation() instead.
          } else {
        ...
        // Like setSelectedChatId, but skips the conversations-list lookup —
        // use for a chat that was just created, to avoid racing its sync.
        openConversation: (otherUserId, conversationRef) => {
          set({ selectedChatId: otherUserId, selectedProfile: null })
          activateConversation(otherUserId, conversationRef)
        },
        ...
        unsubscribeFromMessages: () => {
          const { subscriptions, conversationSubscriptions, currentChat } =
            get()
